...
/**
 * Handles all CRUD operations for /api/mealPlan
 */
public class MealPlanHandler implements HttpHandler {      
    private String path;
    private final MealPlanService service;
    
    public MealPlanHandler(MealPlanService service) {
    ...
    private void handleGetAll(HttpExchange exchange, String owner_id) throws IOException {
        ...
        // Get and send mealPlans
        List <MealPlan> mealPlans = service.getAllMealPlans(owner_id);
        sendJson(exchange, 200, mealPlans);
    }
    ...
    private void handleGetById(HttpExchange exchange, int id, String owner_id) throws IOException {
        ...
        Optional<MealPlan> mealPlan = service.getMealPlanById(owner_id, id);
        String requestId = Logger.newRequestId();
        // Log request
        Logger.info(String.format(
        "Incoming request %s %s", exchange.getRequestMethod(), exchange.getRequestURI()),
        ...
        if (mealPlan.isPresent()) {
            ...
            MealPlan m = mealPlan.get();
            JSONObject obj = new JSONObject();
            obj.put("id", m.getId());
            obj.put("name", m.getName());
            obj.put("startDate",m.getStartDate());
            obj.put("endDate",m.getEndDate());
            obj.put("entries", m.getEntries());
    ...
    private void handleCreate(HttpExchange exchange, String owner_id) throws IOException {
        ...
        MealPlan mealPlan;
        try {
            mealPlan = gson.fromJson(body, MealPlan.class);
            System.out.println("Parsed name = " + mealPlan.getName());
        } catch (Exception e) {
            e.printStackTrace();
            Main.sendText(exchange, 400, "{ \"error\": \"JSON parse failed: " + e.getMessage() + "\" }");
            return;
        }
        mealPlan.setOwnerId(owner_id);
        try {
            MealPlan created = service.createMealPlan(mealPlan);
     ...
     * @throws IOException
     */
    private void handleUpdate(HttpExchange exchange, int id, String owner_id) throws IOException {
        ...
        mealPlan.setOwnerId(owner_id);
        try {
            boolean updated = service.updateMealPlan(id, mealPlan, owner_id);
            if (updated) {
                // Record metrics
                // long latency = System.currentTimeMillis() - start;
                // Metrics.recordRequest(path, 404);
